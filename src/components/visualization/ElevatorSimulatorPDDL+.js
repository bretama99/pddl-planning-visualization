...
function parseActionByType(line, index, pddlType) {
  ...
  const planPassengers = computed(() => {
   ...
   const planFloors = computed(() => {
    ...
    // FIXED: For numerical and PDDL+ - correct floor calculation with consecutive numbering starting from Floor1
    else {
      const moveActions = parsedActions.value?.filter(a => 
        a.actionType === 'move' || a.name?.includes('move')
      ) || []
      
      console.log(`🏢 ${pddlType.toUpperCase()}: Analyzing ${moveActions.length} movement actions for floor calculation`)
      
      // Track elevator position through the plan
      let currentFloor = 0 // Start at floor index 0
      let minFloor = 0
      let maxFloor = 0
      ...
      moveActions.forEach((action, index) => {
        ...
        console.log(`🏢 ${pddlType}: Step ${index + 1}: ${action.name} -> Floor index ${currentFloor} (range: ${minFloor} to ${maxFloor})`)
      })
      
      // CORRECTED: Calculate total floors needed and generate consecutive Floor1, Floor2, etc.
      const totalFloorsNeeded = maxFloor - minFloor + 1
      
      console.log(`🏢 ${pddlType}: Floor range from index ${minFloor} to ${maxFloor} = ${totalFloorsNeeded} floors needed`)
      
      // Generate consecutive floor names starting from Floor1
      for (let i = 1; i <= totalFloorsNeeded; i++) {
        floors.add(`Floor${i}`)
      }
      
      // Ensure minimum of 3 floors for safety
      if (floors.size < 3) {
        floors.clear()
        for (let i = 1; i <= 3; i++) {
          floors.add(`Floor${i}`)
    ...
    console.log(`🏢 Final floors for ${pddlType}:`, result)
    return result
  })

  const movementPaths = computed(() => {
  if (!parsedActions.value?.length || props.pddlType === 'classical' || props.pddlType === 'temporal') {
    return new Map() // Don't calculate for Classical/Temporal - they're already smooth
  }
  
  console.log(`🎯 CALCULATING MOVEMENT PATHS for ${props.pddlType.toUpperCase()}`)
  
  const paths = new Map()
  const elevatorPositions = {}
  
  // Initialize elevator starting positions
  planElevators.value.forEach(elevator => {
    elevatorPositions[elevator] = planFloors.value[0] // Start at first floor
    
    // Adjust starting position based on first move direction
    ...
    if (firstMoveAction && (firstMoveAction.direction === 'down' || firstMoveAction.name === 'move-down')) {
      // If first move is down, start from a higher floor
      elevatorPositions[elevator] = planFloors.value[planFloors.value.length - 1]
    }
  })
  
  console.log(`🎯 Initial elevator positions:`, elevatorPositions)
  
  // Calculate movement paths for each action
  parsedActions.value.forEach((action, index) => {
    if (action.actionType === 'move' || action.name?.includes('move')) {
      const elevator = action.elevator || 'elevatorx'
      const currentFloor = elevatorPositions[elevator]
      ...
      if (direction === 'up' || action.name === 'move-up') {
        targetIndex = Math.min(currentIndex + 1, planFloors.value.length - 1)
      } else if (direction === 'down' || action.name === 'move-down') {
      ...
      // Update elevator position for next calculation
      elevatorPositions[elevator] = targetFloor
      
      console.log(`🎯 Action ${index}: ${elevator} moves ${currentFloor} → ${targetFloor} (${direction})`)
    }
  })
...
function startSmoothAnimationLoop() {
  ...
  function animateSmooth() {
    ...
    // Enhanced passenger movements with smoother easing
    for (const passenger of Object.keys(passengerMovement.value)) {
      ...
      if (startTime) {
        ...
        // ENHANCED: Different easing curves for different PDDL types
        if (props.pddlType === 'pddl+') {
          // Smooth sine curve for PDDL+
          progress = 0.5 * (1 - Math.cos(progress * Math.PI))
        } else if (props.pddlType === 'numerical') {
          // Smooth cubic for Numerical
          progress = progress * progress * (3 - 2 * progress)
        } else {
          // Keep original for Classical/Temporal
          progress = progress * progress * (3 - 2 * progress)
        }
    ...
    // ENHANCED: Smoother elevator movement for all types
    for (const elevator of Object.keys(elevatorSmoothPositions.value)) {
      ...
      if (distance > 0.5) {
        ...
        // ENHANCED: Adaptive speed for smoother movement
        if (props.pddlType === 'pddl+') {
          // Faster acceleration, smoother deceleration for PDDL+
          speed = Math.min(0.18, 0.06 + distance * 0.003)
        } else if (props.pddlType === 'numerical') {
          // Consistent speed for Numerical
          speed = Math.min(0.15, 0.05 + distance * 0.002)
        } else {
          // Keep original speed for Classical/Temporal
          speed = 0.1
        }
        
        const direction = targetPosition > currentPosition ? 1 : -1
        const moveAmount = Math.min(distance, Math.max(2, distance * speed))
...
function executeLoad(action) {
  ...
  setTimeout(() => {
    ...
    if (!animationFrame) startSmoothAnimationLoop()
  }, doorOpenTime)
...
function executeMovement(action) {
  ...
  console.log(`🎯 MOVEMENT EXECUTION: ${props.pddlType.toUpperCase()}`, {
    action: action.name,
    elevator,
    currentFloor,
    direction: action.direction
  ...
  // UNCHANGED: Classical and Temporal use explicit floor parameters (keep smooth)
  if (props.pddlType === 'classical' || props.pddlType === 'temporal') {
    currentFloor = action.fromFloor || currentFloor
    targetFloor = action.toFloor
    console.log(`📋 ${props.pddlType.toUpperCase()}: Direct movement ${currentFloor} → ${targetFloor}`)
  } 
  // ENHANCED: PDDL+ and Numerical use pre-calculated smooth paths
  else {
    const actionIndex = parsedActions.value.indexOf(action)
    const movementPath = movementPaths.value.get(actionIndex)
    
    if (movementPath) {
      // Use pre-calculated smooth path
      targetFloor = movementPath.toFloor
      console.log(`🎯 ${props.pddlType.toUpperCase()}: Using smooth path ${movementPath.fromFloor} → ${targetFloor}`)
    } else {
      // Fallback: Calculate target floor step by step
      const currentIndex = planFloors.value.indexOf(currentFloor)
      if (currentIndex === -1) {
        console.error(`❌ Current floor ${currentFloor} not found in floors:`, planFloors.value)
        return
      }
      ...
      console.log(`🎯 ${props.pddlType.toUpperCase()}: Fallback calculation ${currentFloor} → ${targetFloor}`)
    }
  }
  
  if (!targetFloor) {
    console.error(`❌ No target floor determined for:`, action)
    return
  }
  
  if (targetFloor === currentFloor) {
    console.log(`ℹ️ Already at target floor ${targetFloor}, skipping movement`)
    return
  }
  
  // Calculate movement duration
  let movementDuration = action.duration || getDurationForType(props.pddlType, 'move')
  
  if (props.pddlType === 'classical') {
    // Classical: Calculate duration from plan timing
    const actionIndex = parsedActions.value.indexOf(action)
    ...
    // Temporal: Use explicit duration from [X.X]
    movementDuration = action.duration || TIMING.temporal.FLOOR_TRAVEL_TIME
  } else {
    // ENHANCED: PDDL+/Numerical use consistent timing for smooth movement
    movementDuration = action.duration || TIMING[props.pddlType]?.FLOOR_TRAVEL_TIME || 1.5
  }
  
  console.log(`⏱️ Movement duration: ${movementDuration}s`)
  ...
  nextTick(() => {
    ...
    if (elevatorSmoothPositions.value[elevator] === undefined) {
      const fromIndex = planFloors.value.indexOf(currentFloor)
      elevatorSmoothPositions.value = {
        ...elevatorSmoothPositions.value,
        [elevator]: fromIndex * 60
      }
    }
    
    console.log(`🛗 STARTING SMOOTH MOVEMENT: ${elevator} from ${currentFloor} to ${targetFloor}`)
    
    // ENHANCED: Use smooth animation for all types
    startSmoothAnimationLoop()
    
    // Complete movement after calculated duration
    setTimeout(() => {
      ...
      console.log(`✅ MOVEMENT COMPLETED: ${elevator} now at ${targetFloor} in ${movementDuration}s`)
